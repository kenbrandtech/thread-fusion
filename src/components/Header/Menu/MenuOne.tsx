...
>>>>>>> 019d1c70e78b17f62bfb1a64436070ae0a796abc
    ...
    useEffect(() => {
        axios
            .get(`${process.env.NEXT_PUBLIC_API_URL}/sub-category/1`, {
            ...
            <div className="container mx-auto h-full">
                <div className="header-main flex justify-between h-full">
                    ...
                    <div className="left flex items-center gap-16">
                        <Link href="/">
                            <img src="/images/logo/main-logo.png" style={{ height: "100px", marginTop: "20px" }} alt="main-logo" />
                        </Link>
                        <div className="menu-main h-full max-lg:hidden">
                            <ul className="flex items-center gap-8 h-full">
                                {menuItems.map((item) => (
                                    <li
                                        key={item.id}
                                        className="h-full relative group" // Use `group` for hover functionality
                                    >
                                        <Link href={`/shop/${item.slug}`} className={`text-button-uppercase duration-300 h-full flex items-center justify-center ${pathname.includes(`/shop/${item.slug}`) ? "active" : ""}`}>
                                            {item.name}
                                        </Link>

                                        {/* Sub-menu */}
                                        {item.subcategory && item.subcategory.length > 0 && (
                                            <div className="sub-menu py-3 px-5 -left-10 w-max absolute grid grid-cols-4 gap-5 bg-white rounded-b-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity duration-300">
                                                <ul>
                                                    {item.subcategory.map((sub) => (
                                                        <li key={sub.id}>
                                                            <Link href={`/subcategory/${sub.slug}`} className={`link text-secondary duration-300 ${pathname.includes(`/subcategory/${sub.slug}`) ? "active" : ""}`}>
                                                                {sub.name}
                                                            </Link>
                                                        </li>
                                                    ))}
